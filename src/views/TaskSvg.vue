    ...
    const getWorkflow = async () => {
      try {
        ...
        Object.keys(props.stepsList).forEach(t => {
          ...
          const desc = props.stepsList[t].description
          g.setNode(t, {
            id: t,
            label: t,
            description: desc,
          ...
          props.stepsList[t].dependencies?.forEach((d: string) => {
            let label = ''
            if (d.endsWith(':ANY')) {
              d = d.slice(0, -4)
              label = 'ANY'
        ...
        if (Object.keys(props.stepsList).length < 5) {
          initialScale = 0.9
        } else if (Object.keys(props.stepsList).length < 9) {
          initialScale = 0.8
        }
        ...
        svg.call(
          ...
          d3.zoomIdentity
            .translate(Math.abs((graphWidth - width) * initialScale ) / 2, 0)
            .scale(initialScale)
        )
